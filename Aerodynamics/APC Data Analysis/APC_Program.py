...
def collect_data(
    ...
    for i, (segment_name, segment_df) in enumerate(segment_dfs_raw.items()):
        ...
        if (
            variable_to_hold_constant not in segment_df.columns
            or selected_variable not in segment_df.columns
        ):
            print(
                f"Variable '{variable_to_hold_constant}' or '{selected_variable}' not found in {segment_name}. Skipping this segment."
            )
            continue
        # Filter rows where the variable to hold constant is close to the specified value
        matching_rows = segment_df[
            (segment_df[variable_to_hold_constant] - variable_value).abs() < tolerance
        ]
        ...
        if not matching_rows.empty:
            ...
            if not positive_diff.empty and not negative_diff.empty:
                ...
                factor = (variable_value - row_below[variable_to_hold_constant]) / (
                    row_above[variable_to_hold_constant]
                    - row_below[variable_to_hold_constant]
                )

                # Interpolate the selected variable
                interpolated_value = row_below[selected_variable] + factor * (
                    row_above[selected_variable] - row_below[selected_variable]
                )
                selected_row = pd.Series(
                    segment_df.columns, index=segment_df.columns
                )  # Create an empty Series
                selected_row[selected_variable] = interpolated_value
            else:
                # Interpolation cannot be performed
                print(
                    f"Cannot interpolate for {segment_name} at RPM {rpm_value} ({variable_to_hold_constant} does not cross {variable_value})."
                )
        ...
        print(
            f"[{label}] At RPM {rpm_value}, interpolated {selected_variable} = {selected_row[selected_variable]}"
        )

    # Create a DataFrame from the collected data
    plot_df = pd.DataFrame(
        {"RPM": rpm_list, selected_variable: selected_variable_values}
    )
...
def plot_xy_at_rpm(df, segment_dfs_raw, rpm_values):
    ...
    if x not in available_columns or y not in available_columns:
        print(
            "Selected variables not found in the data. Please check the variable names and try again."
        )
        return
...
"""
    ...
    if choice == "1":
        ...
        for i in range(1, num_files + 1):
            ...
            csv_file_path = input(
                f"Paste desired destination file path {i} (filepath/filename.csv): "
            )
            dat_file_paths.append(dat_file_path)
            csv_file_paths.append(csv_file_path)

        # Ask for variable to hold constant and its value
        variable_to_hold_constant = input(
            "Enter the variable to hold constant (e.g., 'Cp', 'V(mph)', etc.): "
        )
        variable_value = float(
            input(f"Enter the value of {variable_to_hold_constant} to hold constant: ")
        )

        # Ask for variable to plot against RPM
        selected_variable = input(
            "Enter the variable you want to plot against RPM (e.g., 'Thrust(Lbf)', 'FOM', etc.): "
        )
        ...
        for i in range(num_files):
            label = f"DataSet {i+1}"
            print(f"\nProcessing {label}...")
            df, segment_dfs_raw, rpm_values = process_dat_file(
                dat_file_paths[i], csv_file_paths[i]
            )
        ...
        plt.title(
            f"{selected_variable} vs RPM at {variable_to_hold_constant} = {variable_value}"
        )
        plt.grid(True)
        plt.legend()
        plt.show()
        ...
        csv_file_path = input(
            "Paste desired destination file path (filepath/filename.csv): "
        )
