...
if start_idx is not None:
    next_rpm_index = rpm_rows[rpm_rows > start_idx].min() if any(rpm_rows > start_idx) else df.shape[0]# Find the next RPM index to determine the end of the current RPM section
    rpm_data = df.iloc[start_idx:next_rpm_index].reset_index(drop=True)# Slice the data for the current RPM range
    ...
    plt.grid(True)
    plt.xlim([x_data.min() - 0.1 * abs(x_data.min()), x_data.max() + 0.1 * abs(x_data.max())]) # To adjust plot grid size based on range
    plt.ylim([y_data.min() - 0.1 * abs(y_data.min()), y_data.max() + 0.1 * abs(y_data.max())])
